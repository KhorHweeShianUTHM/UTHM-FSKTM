...
<div id="notification-popup" class="notification-popup" hidden>
  <div class="popup-header">
    <h2>Notifications</h2>
    <a href="notification.html" class="mark-all-read">Mark all as read</a>
  </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const profileIcon = userProfile.querySelector('.dropdown-icon i');
    ...
    function renderNotifications(notifications) {
        ...
        if (!notifications.length) {
            notificationListContainer.innerHTML = `<div class="no-notifications">You're all caught up!</div>`;
            return;
        }
        ...
        notificationListContainer.appendChild(ul);
    }
    ...
    userProfile.addEventListener('click', e => {
        ...
        const isProfileDropdownHidden = profileDropdown.hasAttribute('hidden');

        // Close notification popup if open
        if (!notificationPopup.hasAttribute('hidden')) {
            notificationPopup.setAttribute('hidden', '');
            // Optional: Remove active class from notification icon if you have one
            // notificationIcon.classList.remove('active');
        }

        // Toggle profile dropdown visibility
        profileDropdown.toggleAttribute('hidden');
        profileIcon.classList.toggle('fa-chevron-down', !isProfileDropdownHidden);
        profileIcon.classList.toggle('fa-chevron-up', isProfileDropdownHidden);
        userProfile.classList.toggle('active', isProfileDropdownHidden);
    });
    ...
    notificationIcon.addEventListener('click', async (e) => {
        ...
        const isNotificationPopupHidden = notificationPopup.hasAttribute('hidden');

        // Close profile dropdown if open
        if (!profileDropdown.hasAttribute('hidden')) {
            profileDropdown.setAttribute('hidden', '');
            profileIcon.classList.replace('fa-chevron-up', 'fa-chevron-down');
        ...
        // Fetch and render notifications only when opening the popup
        if (isNotificationPopupHidden) {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!profileDropdown.hasAttribute('hidden') && !userProfile.contains(e.target)) {
            profileDropdown.setAttribute('hidden', '');
            profileIcon.classList.replace('fa-chevron-up', 'fa-chevron-down');
            userProfile.classList.remove('active');
        }
        ...
        if (!notificationPopup.hasAttribute('hidden') && !notificationIcon.contains(e.target) && !notificationPopup.contains(e.target)) {
            notificationPopup.setAttribute('hidden', '');
        }
    });
});
