...
<style>
  ...
  .pagination a.disabled {
    ...
    background-color: white;
    border: 1px solid #ccc;
    padding: 8px 40px 8px 12px; /* Space for the arrow */
    font-size: 16px;
    border-radius: 4px;
  ...
  /* Optional: Remove arrow for IE (if needed) */
  select::-ms-expand {
    display: none;
  }
</style>
...
<div class="sidebar">
  ...
  <ul class="menu-top">
    ...
    <li  class="active" style="margin-left: 3px;"><a href="admin_jobcards.php"><i class="fas fa-file-alt" style="margin-right: 12px;"></i> Job Cards</a></li>
...
<div class="main">
  ...
  <div class="navbar">
    <div class="page-info">
        <h1>Jobcards Management</h1>
        <p>Manage your jobcards details</p>
    </div>
    ...
    <div class="content">
      <div class="top-bar">
        <form method="GET" action="" style="display: flex; gap: 10px;">
          ...
          <button type="button" onclick="openModal()"><i class="fas fa-plus"></i> Add Jobcards</button>
        </form>
      </div>
      ...
      <table>
        ...
        <tbody>
          ...
          $count_sql = "SELECT COUNT(*) AS total FROM job_cards";
          if (!empty($search)) {
          ...
          $sql = "SELECT job_id, name, vehicles, date_in, date_out, status, service, remarks, amount, created_at, updated_at FROM job_cards";
          if (!empty($search)) {
          ...
          if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                  ...
                  echo "<td><strong>" . date("d F Y", strtotime($row['updated_at'])) . "</strong></td>";
                  echo "<td class='actions'>
                          <i class='fas fa-eye' onclick='viewJobcard(" . json_encode($row) . ")'></i>
                          <i class='fas fa-pen' onclick='editJobcard(" . json_encode($row) . ")'></i>
                          <i class='fas fa-trash' onclick='confirmDelete(" . $row['job_id'] . ")'></i>
                        </td>";
                  echo "</tr>";
              }
          } else {
              echo "<tr><td colspan='7'>No jobcard found.</td></tr>";
          }
    ...
    <div id="addJobcardModal" class="modal">
      <div class="modal-content">
        ...
        <h2>Add New Jobcard</h2>
        <form method="POST" action="admin_jobcards_add.php" id="jobcardForm">
          <div class="form-group">
            <label style="margin-top: 25px;">Customer Name</label>
            <input type="text" name="name" required>
          </div>
          <!-- <div class="form-group">
            <label>Vehicle</label>
            <input type="text" name="vehicles" required>
          </div> -->
          <div class="form-group">
            <label>Date In</label>
          ...
          <button type="submit" class="submit-btn">Add Jobcard</button>
        </form>
      </div>
    </div>

    <!-- View Jobcard Modal -->
    <div id="viewJobcardModal" class="modal">
      <div class="modal-content">
        <span class="close-btn" onclick="closeViewModal()">&times;</span>
        <h2 style="margin-bottom: 25px;">Jobcard Details</h2>
        <div id="jobcardDetails" class="customer-info-grid">
          <div class="info-box">
            <i class="fas fa-user"></i>
            <div><strong>Customer Name:</strong><br><span id="viewName"></span></div>
          </div>
          <div class="info-box">
    ...
    <div id="editJobcardModal" class="modal">
      <div class="modal-content">
        ...
        <h2>Edit Jobcard Details</h2>
        <form method="POST" action="admin_jobcards_update.php" id="editJobcardForm">
          <input type="hidden" id="editJobcardId" name="job_id">
          ...
          <div class="form-group">
            ...
            <input type="text" name="vehicles" id="editVehicles" required>
          </div>
          <div class="form-group">
            <label>Date in</label>
            <input type="date" name="date_in" id="editDateIn" required>
          </div>
          <div class="form-group">
            <label>Date Out</label>
            <input type="date" name="date_out" id="editDateOut" required>
          </div>
          <div class="form-group">
            <label>Status</label>
          ...
          <div class="form-group">
            <label>Remarks</label>
            <input type="text" name="remarks" id="editRemarks" required>
          </div>
          <div class="form-group">
            <label>Amount</label>
    ...
    <div id="deleteConfirmModal" class="modal">
      <div class="modal-content">
        ...
        <form method="POST" action="admin_jobcards_delete.php">
          <input type="hidden" name="job_id" id="deleteJobcardId" />
  ...
  const modal = document.getElementById("addJobcardModal");
  ...
  function viewJobcard(data) {
    ...
    const modal = document.getElementById("viewJobcardModal");
    modal.style.display = "flex";
    modal.classList.add("show");
    modal.classList.remove("hide");
  }
  ...
  window.onclick = function(event) {
    ...
    if (event.target == document.getElementById("viewJobcardModal")) {
      closeViewModal();
    }
  }

  let currentJobcardData = null;

function viewJobcard(data) {
  currentJobcardData = data; // store current customer data globally
  ...
  document.getElementById("viewEditBtn").onclick = function () {
    editJobcard(currentJobcardData);
  };
}
    ...
    function editJobcard(data) { 
      document.getElementById("editJobcardId").value = data.job_id;
      document.getElementById("editName").value = data.name;
      document.getElementById("editVehicles").value = data.vehicles;
      document.getElementById("editDateIn").value = formatDate(data.date_in);
      document.getElementById("editDateOut").value = formatDate(data.date_out);
    ...
    document.getElementById('deleteJobcardId').value = job_id;
